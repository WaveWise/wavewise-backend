class ApplicationController < ActionController::API
  ...
  def weather(short_code)
    ...
    @today_weather = @weather_data.select { |key, value| key['date'] == @date_string }.first
    @hour_weather = @today_weather.dig('hourly').select { |key, value| key['time'] == @hour_string }.first
    @tides = @today_weather.dig('tides')
  end

  def condition(short_code)
